  ...
  tabItem(tabName = "reg",
          sidebarLayout(
            sidebarPanel(selectInput(inputId = ns("selector1"),"Seleccion de set de datos",choices = c("iris"="iris","beaver1"="beaver1","mtcars"="mtcars","trees"="trees")),
                         ...
                         radioGroupButtons(
                           inputId = ns("selector2"),
                           label = "Selección de color", 
                           choices = c("viridis"="viridis","magma"="magma","cividis"="cividis","plasma"="plasma"),
                           status = "primary"
                         ),
                         ...
                         awesomeRadio(
                           inputId = ns("reg"),
                           label = "Radio buttons", 
                           choices = c("Lineal"="lin", "No Lineal"="nolin"),
                           selected = "lin",
                           inline = TRUE, 
                           status = "success"
                         ),
            ...
            mainPanel(box(plotOutput(ns("graf1")),width=12,collapsible = FALSE,elevation = 2,headerBorder = FALSE))),
...
reg_sv <- function(input, output, session) {
  ...
  observeEvent(input$reg, priority=1,{
  ...
  if(reg=="lin"){
    ...
    mydata <- base()
    modelo<-lm(data = mydata,mydata[,1]~poly(mydata[,2],2))
    paste0("R2: ",round(summary(modelo)$r.squared,2),br(),br(),
          "R2 ajustado x: ",round(summary(modelo)$adj.r.squared,2),br(),
          "Ajuste lineal: Y=",round(modelo$coefficients[1],2),"+",round(modelo$coefficients[2],2),"X^2",br())
    }
    }else{}    
  })
  })
